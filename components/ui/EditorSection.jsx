...
const EditorSection = ({ title, icon, badge, children, defaultActive, onExpand }) => {
	...
	return (
		<div ref={sectionRef} className={classNames('section border-b border-white/10', { active: isActive })}>
			<div className='flex gap-4 items-center py-5 text-white uppercase text-sm font-bold cursor-pointer' onClick={toggleActive}>
			...
			<div className={classNames('flex flex-col gap-4 py-5 text-md', { hidden: !isActive })}>{children}</div>
		</div>
	)
}
