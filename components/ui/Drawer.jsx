...
const Drawer = ({ id, open: controlledOpen, defaultOpen = true, onToggle, className = '', isVertical = true, children }) => {
    ...
    return (
        <div
            id={id}
            ref={drawerRef}
            className={classNames('fixed z-20 transform-gpu', isVertical ? 'inset-y-0 left-0 w-72' : 'inset-x-0 bottom-0', className)}
            style={{
            ...
            <div
                ...
                className={classNames(
                    'absolute flex items-center justify-center text-black/80 rounded pointer-events-auto cursor-pointer touch-none',
                    isVertical ? 'left-full top-4 ml-4 h-6 w-6' : 'bottom-full left-4 mb-4 h-11 w-11 bg-black/50 text-white p-3'
                )}>
