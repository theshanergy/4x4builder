...
const DevConsole = () => {
	...
	const handleChange = (e) => {
		...
		setInput(val)
		setError(null)
		setSuggestion(val.trim() ? (Object.keys(COMMANDS).find((cmd) => cmd.startsWith(val.trim().toLowerCase())) ?? null) : null)
	}
	...
	return (
		<div className='fixed inset-0 z-100 flex justify-center items-start pt-12 px-4' onClick={() => setOpen(false)}>
					...
					<div className='flex items-center gap-3 px-4 py-3'>
						<div className='relative flex-1'>
							...
							<input
								...
								spellCheck={false}
								autoComplete='off'
								className='w-full bg-transparent border-none text-white text-sm font-mono outline-none placeholder:text-white/20'
							/>
						</div>
