...
const DroneCamera = () => {
	...
	useEffect(() => {
		const initAudio = async () => {
			if (!audioEngineRef.current) {
				audioEngineRef.current = new DroneAudio()
				try {
	...
	useFrame((state, delta) => {
		...
		// Update drone audio based on velocity and altitude
		if (audioEngineRef.current?.isInitialized) {
			const velocityMagnitude = velocity.current.length()
			const altitude = currentPosition.current.y - vehicleState.position.y
			audioEngineRef.current.updateParams(velocityMagnitude, Math.max(0, altitude))
		}
	})
