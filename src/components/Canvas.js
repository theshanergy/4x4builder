...
class VehicleCanvas extends Component {
    ...
    handleConfigChange = (prevProps) => {
        ...
        if (prevProps.vehicle.color !== this.props.vehicle.color || prevProps.vehicle.roughness !== this.props.vehicle.roughness) {
            this.setObjectColor(this.bodyContainer)
        }
        // Lift height.
        ...
        // Wheel size.
        if (
            prevProps.vehicle.rim_diameter !== this.props.vehicle.rim_diameter ||
            prevProps.vehicle.rim_width !== this.props.vehicle.rim_width ||
            prevProps.vehicle.tire_diameter !== this.props.vehicle.tire_diameter
        ) {
            this.setWheelSize()
        }
    ...
    sceneSetup = () => {
        ...
        if (!isMobileOnly) {
            ...
            import('three/examples/jsm/objects/Reflector.js').then(({ Reflector }) => {
                ...
                groundMirror.name = 'GroundReflection'
        ...
        this.renderer.setAnimationLoop(() => {
            ...
            if (this.renderer.xr.isPresenting) {
                // Rotate vehicle.
                if (this.rotateVehicle) {
                    this.vehicleContainer.rotation.y += this.rotateVehicle
                }
                // Walk.
                if (this.walk) {
                    this.xrRig.position.z += this.walk
                    if (this.xrRig.position.z < 1) {
                        this.xrRig.position.z = 1
                    }
                }
            }
    ...
    setupXR = () => {
        ...
        navigator.xr.isSessionSupported('immersive-vr').then((isSupported) => {
            this.vrSupported = isSupported
        })

        // Get displays.
        if ('getVRDisplays' in navigator) {
            navigator.getVRDisplays().then(function (displays) {
                this.renderer.xr.setDevice(displays[0])
            })
        }
        ...
        this.xrRig.position.set(0, 0, 4)
        this.scene.add(this.xrRig)

        // Controllers
        this.controller1 = this.renderer.xr.getController(0)
        this.controller1.addEventListener('selectstart', () => (this.walk = -0.1))
        this.controller1.addEventListener('selectend', () => (this.walk = 0))
        this.controller1.addEventListener('squeezestart', () => (this.rotateVehicle = 0.1))
        this.controller1.addEventListener('squeezeend', () => (this.rotateVehicle = 0))
        this.xrRig.add(this.controller1)

        this.controller2 = this.renderer.xr.getController(1)
        this.controller2.addEventListener('selectstart', () => (this.walk = 0.1))
        this.controller2.addEventListener('selectend', () => (this.walk = 0))
        this.controller2.addEventListener('squeezestart', () => (this.rotateVehicle = -0.1))
        this.controller2.addEventListener('squeezeend', () => (this.rotateVehicle = 0))
        ...
        this.renderer.xr.addEventListener('sessionstart', () => {
            ...
            // Rotate vehicle.
            this.vehicleContainer.rotateY(THREE.Math.degToRad(-35))
        })
        // XR session end.
        this.renderer.xr.addEventListener('sessionend', () => {
            // Remove camera to xrRig.
            this.xrRig.remove(this.camera)
        })
    }
    ...
    loadVehicle = () => {
        ...
        this.loader.load(vehicleConfigs.vehicles[this.props.vehicle.id].model, (vehicleModel) => {
            ...
            if (typeof this.vehicle !== 'undefined') {
                this.bodyContainer.remove(this.vehicle)
            }
            ...
            this.bodyContainer.position.y = height + 0.1 // add a little extra for a 'drop in' effect.
            this.setVehiclePosY()
            ...
            this.bodyContainer.add(this.vehicle)
        })
    }
    ...
    setVehiclePosY = () => {
        if (typeof this.vehicle !== 'undefined') {
            ...
            new TWEEN.Tween(this.bodyContainer.position).to({ y: posY }, 1000).easing(TWEEN.Easing.Elastic.Out).start()
        }
    }
    ...
    render() {
        return (
            <div id='vehicle' ref={(ref) => (this.mount = ref)}>
                ...
                <div id='actions'>
                    {this.vrSupported && (
                        <button id='vr-button' onClick={this.startVR}>
                            VR
                        </button>
                    )}
