...
export class QuadtreeNode {
	 ...
	 */
	shouldSubdivide(viewerX, viewerZ, splitFactor, minSize, vehicleX, vehicleZ) {
		// Don't subdivide if we're at minimum size
		if (this.size <= minSize) return false

		// Use the closer of viewer and vehicle so physics tiles exist
		// under the vehicle even when the camera is far (e.g. drone).
		const dxV = viewerX - this.centerX
		const dzV = viewerZ - this.centerZ
		let distSq = dxV * dxV + dzV * dzV

		if (vehicleX !== undefined) {
			const dxP = vehicleX - this.centerX
			const dzP = vehicleZ - this.centerZ
			const vehicleDistSq = dxP * dxP + dzP * dzP
			if (vehicleDistSq < distSq) distSq = vehicleDistSq
		}
	 ...
	 */
	shouldMerge(viewerX, viewerZ, splitFactor, hysteresis, vehicleX, vehicleZ) {
		const dxV = viewerX - this.centerX
		const dzV = viewerZ - this.centerZ
		let distSq = dxV * dxV + dzV * dzV
	 ...
	 */
	update(viewerX, viewerZ, splitFactor, hysteresis, minSize, vehicleX, vehicleZ) {
		if (this.children) {
			// Already subdivided - check if we should merge
			if (this.shouldMerge(viewerX, viewerZ, splitFactor, hysteresis, vehicleX, vehicleZ)) {
				this.merge()
			} else {
				// Update children recursively
				for (const child of this.children) {
					child.update(viewerX, viewerZ, splitFactor, hysteresis, minSize, vehicleX, vehicleZ)
				}
			}
		} else {
			// Leaf node - check if we should subdivide
			if (this.shouldSubdivide(viewerX, viewerZ, splitFactor, minSize, vehicleX, vehicleZ)) {
				this.subdivide()
