...
const useWaterMaterial = () => {
	...
	useFrame((_, delta) => {
		...
		// Skip reflection if no water tiles are currently visible.
		// The reflection pass re-renders the whole scene, so gating it out
		// on land-only frames is the biggest single frame-time win.
		if (waterVisibility.visibleTileCount === 0) return
